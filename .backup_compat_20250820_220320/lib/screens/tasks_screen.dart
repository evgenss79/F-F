...
class _TasksScreenState extends State<TasksScreen> {
  ...
  void _addTask(String title, {String? assignee}) {
    ...
    if (task.assignee == null) {
      Notify.show("Новая общая задача", "${task.title} (создал: ${task.creator})");
    } else {
      ...
      body: ListView(
        children: [
          const Padding(
            ...
            child: Text("Общие задачи", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          ...generalTasks.map((task) => ListTile(
                title: Text(task.title),
                subtitle: Text("Создатель: ${task.creator}\nСоздана: ${task.createdAt}"),
                trailing: Row(
          ...
          const Padding(
            padding: EdgeInsets.all(8.0),
            child: Text("Индивидуальные задачи", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          ...personalTasks.map((task) => ListTile(
                title: Text(task.title),
                subtitle: Text(
